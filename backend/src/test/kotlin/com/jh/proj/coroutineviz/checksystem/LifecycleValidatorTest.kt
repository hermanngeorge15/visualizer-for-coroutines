...
class LifecycleValidatorTest {
    ...
    private fun started(coroutineId: String, seq: Long, tsNanos: Long = seq * 1000): CoroutineStarted =
        CoroutineStarted(
            sessionId = "test-session",
            seq = seq,
            tsNanos = tsNanos,
    ...
    private fun completed(coroutineId: String, seq: Long, tsNanos: Long = seq * 1000): CoroutineCompleted =
        CoroutineCompleted(
            sessionId = "test-session",
            seq = seq,
            tsNanos = tsNanos,
    ...
    private fun cancelled(coroutineId: String, seq: Long, tsNanos: Long = seq * 1000): CoroutineCancelled =
        CoroutineCancelled(
            sessionId = "test-session",
            seq = seq,
            tsNanos = tsNanos,
    ...
    private fun suspended(coroutineId: String, seq: Long, tsNanos: Long = seq * 1000): CoroutineSuspended =
        CoroutineSuspended(
            sessionId = "test-session",
            seq = seq,
            tsNanos = tsNanos,
    ...
    fun `events after terminal state detected`() {
        val events: List<VizEvent> = listOf(
            ...
            started("c1", 2),
            completed("c1", 3),
            suspended("c1", 4) // This event is after the terminal state
        )
