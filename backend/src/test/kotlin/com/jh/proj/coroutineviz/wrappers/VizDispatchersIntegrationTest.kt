...
class VizDispatchersIntegrationTest {
    ...
    fun testCompleteScenarioWithMixedDispatchers() =
        runBlocking {
            ...
            val apiJob =
                cpuScope.vizLaunch(label = "api-handler") {
                    // Parse request (CPU)
                    vizDelay(50)
    ...
    fun testProducerConsumerWithDispatchers() =
        runBlocking {
            ...
            val consumerJob =
                scope.vizLaunch(label = "consumer", context = dispatchers.io) {
                    repeat(3) { i ->
                        // Simulate I/O
                        vizDelay(100)
    ...
    fun testParallelAsyncTasksWithDispatchers() =
        runBlocking {
    ...
    fun testCustomThreadPoolDispatcher() =
        runBlocking {
    ...
    fun testDeepNestingWithDispatcherSwitches() =
        runBlocking {
    ...
    fun testExceptionHandlingWithDispatchers() =
        runBlocking {
            ...
            // expected simulated failure - handler intentionally ignores exceptions
    ...
    fun testStressTestWithManyCoroutines() =
        runBlocking {
            ...
            repeat(coroutineCount) { index ->
                ...
                val job =
                    scope.vizLaunch(label = "stress-$index", context = dispatcher) {
                        vizDelay(10)
                    }
                jobs.add(job)
            }
            ...
            assertTrue(
                dispatcherEvents.size >= coroutineCount,
                "Should have at least $coroutineCount dispatcher events, got ${dispatcherEvents.size}",
            )
