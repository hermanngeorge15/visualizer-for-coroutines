...
object SyncScenarios {
    ...
    suspend fun bankAccountTransfer(scope: VizScope) {
        ...
        suspend fun transfer(
            ...
            val (first, second) =
                if (from.mutex.mutexId < to.mutex.mutexId) {
                    from to to
                } else {
                    to to from
                }
    ...
    suspend fun deadlockDemonstration(scope: VizScope) {
        ...
        // This WILL deadlock
        val job1 =
            scope.vizLaunch("worker-1") {
                mutexA.withLock {
                    println("Worker 1: acquired A")
                    // Give worker 2 time to acquire B
                    vizDelay(100)
                    mutexB.withLock {
                        // Never reached
                        println("Worker 1: acquired B")
                    }
                }
            }

        val job2 =
            scope.vizLaunch("worker-2") {
                mutexB.withLock {
                    println("Worker 2: acquired B")
                    // Give worker 1 time to acquire A
                    vizDelay(100)
                    mutexA.withLock {
                        // Never reached
                        println("Worker 2: acquired A")
                    }
                }
            }
    ...
    suspend fun ecommerceOrderProcessing(scope: VizScope) {
        ...
        val orders =
            listOf(
                ...
                Order(5, "phone", 5),
                Order(6, "tablet", 3),
                // This will fail - not enough stock
                Order(7, "laptop", 3),
            )
